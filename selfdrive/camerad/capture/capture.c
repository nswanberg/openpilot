...
static void enum_formats(void)
{
        struct v4l2_fmtdesc req;
        do {
        } while (errno
        CLEAR(req);
        if(-1 == xioctl(fd, VIDIOC_ENUM_FMT, &req)) {
          if (ENVAL == errno) {
            fprintf(stderr, "%S does not support ENUM_FORMATS", dev_name);
            exit(EXIT_FAILURE);
          } else {
...
{
        dev_name = "/dev/video33"; //"/dev/video0";

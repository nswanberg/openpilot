 ...
 */
#define VENDOR_ID      0x1b4f   // Arduino LLC
#define PRODUCT_ID     0x9206   // Arduino Leonardo
...
int main(int argc, char **argv)
{
    ...
    int len;
	  ...
	  static const char command[] = "{'type':'steering','value':0}";
    static const int COMMAND_LEN = 29;
    int i;
    while(1) {
        //write_char('t');
        for(i=0;i<COMMAND_LEN;i++)
				{
				    write_char(command[i]);
        }
        len = read_chars(buf, BUF_SIZE-1);
        buf[len] = 0;
